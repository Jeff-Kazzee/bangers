---
name: banger-script-longform
description: >
  Write full long-form video scripts for YouTube (and long-form talking-head/tutorial/reaction content).
  Use whenever the user asks to "write a YouTube script", "script for my video", "long-form video about X",
  "write a video explaining Y", "tutorial script", "reaction video script", "10-minute video on Z", or needs
  a complete narrated script with a retention structure, hook, chapters, and B-roll/visual cues. Part of the
  BANGERS suite; applies Fireship's density, Matt Pocock's teach-one-thing clarity, and Theo/ThePrimeagen's
...
- Match the format to the video type:
  - **Explainer / tutorial / "X in 100 seconds" style** → `${CLAUDE_PLUGIN_ROOT}/references/creators/fireship.md` (density, definition-first hook, the 100-seconds skeleton) + `${CLAUDE_PLUGIN_ROOT}/references/creators/matt-pocock.md` (one concept, show-don't-tell, scaffolding).
  - **Reaction / news / opinion** → `${CLAUDE_PLUGIN_ROOT}/references/creators/theo.md` (react + value-add + verdict) + `${CLAUDE_PLUGIN_ROOT}/references/creators/primeagen.md` (read-a-beat-then-riff, energy, spikes).
- `${CLAUDE_PLUGIN_ROOT}/references/platforms/platform-playbook.md` §1 (YouTube long-form specs, retention benchmarks, CTR/mismatch rules, chapters).
...
- A title + thumbnail suggestion at the top (or hand to `banger-hooks`).
