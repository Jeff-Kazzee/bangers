---
name: banger-explainer
description: >
  Write technical explainers and teaching content: how-it-works posts, tutorials, build logs,
  architecture walkthroughs, "why this breaks" pieces, and developer or practitioner threads and
  essays. Use whenever the user asks to "explain how X works", "write a tutorial", "technical
  thread", "write up this build", "teach this", "explainer post", "walk through my architecture",
  or wants content where the reader must understand a mechanism well enough to repeat it. Part of
  the BANGERS suite. Applies the frame/payload split: the author's voice carries the frame,
  technical clarity carries the explanation. Reach for this whenever the reader has to learn
  something mechanical, not just agree with an opinion.
metadata:
  version: "1.1.0"
  suite: "bangers"
---
...
- `${CLAUDE_PLUGIN_ROOT}/references/frameworks/voice-and-audience.md` — the
  audience profile and the creator's hard constraints.
- `${CLAUDE_PLUGIN_ROOT}/references/frameworks/technical-clarity.md` — the
  frame/payload split and the payload discipline. Core reference here.
- `${CLAUDE_PLUGIN_ROOT}/references/frameworks/writing-quality.md` — the
  living gate, run before delivery.
- The target platform section of
  `${CLAUDE_PLUGIN_ROOT}/references/platforms/platform-playbook.md`.
...
missing or past review, run `banger-research`, label the claim unverified, or
omit it.
...
`banger-threads` or `banger-longform-written` and say so.
...
- **Facebook group** — do not use this procedure. Route to
  `banger-longform-written`, question-led and short.
